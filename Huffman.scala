...
object Huffman {
  ...
  // Part 1: Basics
/**
 * Returns the total weight of the given Huffman Tree.
 */
  ...
  /**
   * Returns the list of characters defined in the given Huffman Tree.
   */
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(left, right, chars, w) => chars
    case Leaf(char, w) => char :: Nil
  }

  /*
   * Combines two CodeTrees into a single tree.
   */
  ...
  def times(chars: List[Char]): List[(Char, Int)] = chars match {
    case Nil => Nil
    case c :: cs => (c, chars.count(_ == c)) :: times(cs.filterNot (_ == c))
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
	...
	makeLeafList(freqs.sortWith((c1, c2) => c1._2 < c2._2))
  }
   ...
   */
  def until[T](s: T => Boolean, c: T => T)(trees: T): T =
    if (s(trees)) trees 
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def decodeChar(tree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
      tree match {
        case Fork(left, right, chars, w) => {
          bits.head match {
            case 0 => decodeChar(left, bits.tail)
            case 1 => decodeChar(right, bits.tail)
          }  
        }
    ...
    decodeChar(tree, bits) match {
      ...
      case (char, remainingBits) => char :: decode(tree, remainingBits)
    }
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case Nil => println("Can't find character ", char); Nil
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def convertAux(tree: CodeTree, table: CodeTable, bits: List[Bit]): CodeTable = tree match {
      case f: Fork => 
        mergeCodeTables(convertAux(f.left, table, bits :+ 0), 
