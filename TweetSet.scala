...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
   def union(that: TweetSet): TweetSet = this.filterAcc(p => true, that)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted:  Tweet = {
    def maxRetweeted(tweet1: Tweet, tweet2: Tweet): Tweet = {
      if (tweet1.retweets > tweet2.retweets) tweet1 else tweet2
    }
    if (left.isEmpty && right.isEmpty) elem
    else if (right.isEmpty) maxRetweeted(left.mostRetweeted,elem)
    else if (left.isEmpty) maxRetweeted(right.mostRetweeted,elem)
    else maxRetweeted(right.mostRetweeted, maxRetweeted(left.mostRetweeted,elem))
  }
}
